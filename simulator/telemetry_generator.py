...
class TelemetryGenerator:
    ...
    def vehicle_params(self, vehicle_id: str) -> Tuple[float, float]:
        """(decay_rate, rated_capacity_kwh) for one vehicle — lets a live
        feed continue the same degradation curve a batch-generated
        vehicle was seeded with, without regenerating its full history."""
    ...
    def generate_cycle_row(
        ...
        """One cycle's telemetry row, given the accumulated stress-cycle
        state coming in. Returns (row, updated_stress_cycles) so a caller
        can chain this cycle-by-cycle — generate_vehicle_telemetry below
        loops it for a full batch history; simulator/live_feed.py calls it
        one cycle at a time to keep appending to an already-seeded fleet."""
        cfg = self.config
